...
public class AddBannerActivity extends AppCompatActivity {
        ...
        FirebaseUtil.getDetails().get()
                .addOnCompleteListener(task -> {
                    ...
                    if (task.isSuccessful() && task.getResult() != null) {
                        ...
                        if (doc.exists() && doc.get("lastBannerId") != null) {

                            bannerId = Integer.parseInt(doc.get("lastBannerId").toString()) + 1;

                        } else {

                            bannerId = 1; // first banner
    ...
    private void addToFirebase() {
        ...
        if(idEditText.getText().toString().isEmpty()){
            idEditText.setError("Id required");
            return;
        }

        if(status == null){
            statusDropDown.setError("Select status");
            return;
        }

        if(descEditText.getText().toString().isEmpty()){
